...
contract PriceFeed is Ownable(msg.sender), CheckContract, LiquityBase, IPriceFeed {
  ...
  function getPrice(address _tokenAddress) public view override returns (uint price, bool isTrusted) {
    ...
    if (!isTrusted && address(altFeed) != address(0)) {
      (uint altPrice, bool altIsTrusted) = altFeed.getPrice(_tokenAddress);
      if (altPrice != 0) {
        price = altPrice;
        isTrusted = altIsTrusted; // trusted state might be changed by alt price feed
